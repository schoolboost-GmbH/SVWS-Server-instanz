			...
			<ui-card icon="i-ri-archive-line" title="Jahrgänge" :is-open="actionJahrgaenge" @update:is-open="(isOpen) => actionJahrgaenge = isOpen">
				<svws-ui-table :items="listJahrgaenge" :no-data="false" :columns="cols">
					<template #cell(id)="{value}">
						<svws-ui-checkbox v-if="!readonly" type="toggle" :model-value="jahrgaenge.includes(value)" headless @update:model-value="updateJahrgaenge(value)" />
					</template>
				</svws-ui-table>
			</ui-card>
			<ui-card icon="i-ri-archive-line" title="Räume" :is-open="actionRaeume" @update:is-open="isOpen => actionRaeume = isOpen">
				<svws-ui-table :columns="colsRaeume" :items="listStundenplanRaeume" v-model:clicked="raum" :selectable="!readonly" v-model="selectedRaeume" :count="listStundenplanRaeume.length > 0">
					<template #cell(kuerzel)="{ rowData }">
						<svws-ui-text-input :readonly :model-value="rowData.kuerzel" @change="kuerzel => (kuerzel !== null) && patchRaum({kuerzel}, rowData.id)" headless required />
					</template>
					<template #cell(groesse)="{ rowData }">
						<svws-ui-input-number :readonly :model-value="rowData.groesse" @change="groesse => (groesse !== null) && patchRaum({groesse}, rowData.id)" headless required />
					</template>
					<template #cell(beschreibung)="{ rowData }">
						<svws-ui-text-input :readonly :model-value="rowData.beschreibung" @change="beschreibung => (beschreibung !== null) && patchRaum({beschreibung}, rowData.id)" headless />
					</template>
		...
		<div class="h-full overflow-y-auto w-full xl:w-1/2 flex flex-col gap-8 pr-4">
			<ui-card icon="i-ri-archive-line" title="Pausenzeiten" :is-open="actionPausenzeiten" @update:is-open="isOpen => actionPausenzeiten = isOpen">
				<div class="flex flex-col gap-4">
					<svws-ui-table :columns="colsPausenzeiten" :items="pausenzeitenSorted" v-model:clicked="zeit" :selectable="!readonly" v-model="selectedPausenzeiten" :count="pausenzeitenSorted.length > 0" v-model:sort-by-and-order="sortByAndOrder">
						<template #cell(wochentag)="{ rowData }">
							<svws-ui-select :readonly :model-value="Wochentag.fromIDorException(rowData.wochentag)" @update:model-value="wochentag => patchPausenzeit({wochentag: Number(wochentag?.id || -1)}, rowData.id)" :items="Wochentag.values()" :item-text="i=>i.beschreibung" headless />
						</template>
						<template #cell(beginn)="{ rowData }">
							<svws-ui-text-input :readonly :model-value="DateUtils.getStringOfUhrzeitFromMinuten(rowData.beginn ?? 0)" @change="beginn => (beginn !== null) && patchPausenBeginn(beginn, rowData.id)" headless />
						</template>
						<template #cell(ende)="{ rowData }">
							<svws-ui-text-input :readonly :model-value="DateUtils.getStringOfUhrzeitFromMinuten(rowData.ende ?? 0)" @change="ende => (ende !== null) && patchPausenEnde(ende, rowData.id)" headless />
						</template>
						<template #cell(klassen)="{ rowData }">
							<svws-ui-multi-select :readonly :model-value="[...rowData.klassen].sort()" @update:model-value="klassen => patchPausenKlassen(klassen, rowData.id)" title="Klassen" :items="[...manager().daten().klasseGetMengeAsList()].map(k=>k.id)" :item-text="klasse => manager().daten().klasseGetByIdOrException(klasse).kuerzel" headless />
						</template>
						<template #actions v-if="!readonly">
			...
			<ui-card icon="i-ri-archive-line" title="Aufsichtsbereiche" :is-open="actionAufsichtsbereiche" @update:is-open="isOpen => actionAufsichtsbereiche = isOpen">
				<svws-ui-table :columns="colsAufsichtsbereiche" :items="listStundenplanAufsichtsbereiche" v-model:clicked="bereich" :selectable="!readonly" v-model="selectedAufsichtsbereiche" :count="listAufsichtsbereiche.length > 0">
					<template #cell(kuerzel)="{ rowData }">
						<svws-ui-text-input :readonly :model-value="rowData.kuerzel" @change="kuerzel => (kuerzel !== null) && patchAufsichtsbereich({kuerzel}, rowData.id)" headless />
					</template>
					<template #cell(beschreibung)="{ rowData }">
						<svws-ui-text-input :readonly :model-value="rowData.beschreibung" @change="beschreibung => (beschreibung !== null) && patchAufsichtsbereich({beschreibung}, rowData.id)" headless />
					</template>
					<template #actions v-if="!readonly">
