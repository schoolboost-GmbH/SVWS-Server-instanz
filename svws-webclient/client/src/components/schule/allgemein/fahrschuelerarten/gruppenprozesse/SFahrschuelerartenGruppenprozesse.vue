<template>
	<div class="page page-grid-cards">
		...
		<div v-if="ServerMode.DEV.checkServerMode(serverMode)" class="flex flex-col gap-4">
			<ui-card v-if="hatKompetenzLoeschen" icon="i-ri-delete-bin-line" title="Löschen" subtitle="Ausgewählte Fahrschülerarten werden gelöscht">
				<template #buttonFooterLeft>
					<svws-ui-button title="Löschen" @click="entferneFahrschuelerarten" :is-loading class="mt-4">
...
<script setup lang="ts">
	...
	async function entferneFahrschuelerarten() {
		isLoading.value = true;
		const [delStatus, logMessages] = await props.deleteFahrschuelerarten();
