			...
			<div class="svws-ui-header--title">
				<div class="svws-headline-wrapper">
					...
					<template v-else-if="activeViewType === ViewType.HINZUFUEGEN">
						<h2 class="svws-headline">Anlegen einer neuen Fahrschülerart</h2>
					</template>
					<template v-else-if="activeViewType === ViewType.GRUPPENPROZESSE">
						<h2 class="svws-headline">Gruppenprozesse</h2>
						<span class="svws-subline">{{ fahrschuelerartenSubline }}</span>
					</template>
				</div>
			</div>
...
<script setup lang="ts">
	...
	import type { FahrschuelerartenAppProps } from "~/components/schule/allgemein/fahrschuelerarten/SFahrschuelerartenAppProps";
	...
	const fahrschuelerartenSubline = computed(() => {
