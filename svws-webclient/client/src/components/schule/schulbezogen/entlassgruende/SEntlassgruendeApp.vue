			...
			<div class="svws-ui-header--title">
				<div class="svws-headline-wrapper">
					...
					<template v-else-if="activeViewType === ViewType.GRUPPENPROZESSE">
						...
						<span class="svws-subline">{{ entlassgruendeSubline }}</span>
					</template>
				</div>
			</div>
...
<script setup lang="ts">
	...
	import type { EntlassgruendeAppProps } from "~/components/schule/schulbezogen/entlassgruende/SEntlassgruendeAppProps";
	...
	const entlassgruendeSubline = computed(() => {
		...
		if (list.size() > 5)
			return `${list.size()} Entlassgrund ausgewählt`;
