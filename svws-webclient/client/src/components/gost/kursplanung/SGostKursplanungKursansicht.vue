			...
			<svws-ui-table :items="GostKursart.values()" :columns disable-footer scroll has-background class="pr-4">
				<template #header v-if="blockungstabelleHidden() === 'nichts'">
					...
					<div role="row" class="svws-ui-tr select-none" :style="gridTemplateColumns">
						...
						<div role="columnheader" class="svws-ui-td svws-sortable-column" @click="kurssortierung.value = (kurssortierung.value === 'kursart') ? 'fach' : 'kursart'" :class="zeigeAufklappzeile ? 'col-span-2' : 'col-span-1'">
							<span>Kurs</span>
						...
						<template v-else>
							<template v-if="hatUpdateKompetenz">
								<template v-for="(schiene, index) in schienen" :key="schiene.id">
									<div @dragover="if (istDropZoneSchiene(schiene)) $event.preventDefault();" @drop="openModalRegelKursartSchiene(schiene)" class="svws-ui-td svws-align-center text-ui-25 hover:text-ui-100 !p-0 relative group" role="columnheader"
						...
						<template v-if="istFachwahlVorhanden(fachwahl.fachwahlen, fachwahl.kursart)">
							<template v-if="listeDerKurse(fachwahl).isEmpty() && (getAnzahlFachwahlen(fachwahl) !== 0) && istVorlage && hatUpdateKompetenz">
								<template v-if="blockungstabelleHidden() === 'nichts'">
									<div role="row" class="svws-ui-tr svws-disabled-soft select-none" :style="getGridTemplateColumnsWithBackgroundColor(fachwahl)" :key="fachwahl.kursart.id">
										...
										<div role="cell" class="svws-ui-td" />
										<div role="cell" class="svws-ui-td text-uistatic!">{{ fachwahl.fachwahlen.kuerzel }}-{{ fachwahl.kursart.kuerzel }}</div>
										<div role="cell" class="svws-ui-td" />
										<div role="cell" class="svws-ui-td" />
										<div role="cell" class="svws-ui-td svws-align-center text-uistatic!" @click="toggleSchuelerFilterFachwahl(fachwahl)">
										...
										<div role="cell" class="svws-ui-td svws-align-center" :style="{'gridColumn': 'span ' + getDatenmanager().schieneGetListe().size()}">
											<svws-ui-button :disabled="!hatUpdateKompetenz" type="transparent" @click="hatUpdateKompetenz && add_kurs(fachwahl)" title="Kurs anlegen">
												<span class="inline-flex items-center text-button -mr-0.5">
													<span class="icon-uistatic icon i-ri-book-2-line" />
													<span class="icon-uistatic icon-sm i-ri-add-line -ml-0.5 text-sm" />
												</span>
												<div class="text-uistatic">Kurs anlegen</div>
											</svws-ui-button>
										</div>
									</div>
								</template>
							...
							<template v-else>
								<template v-for="kurs in listeDerKurse(fachwahl)" :key="kurs.id">
									<div role="row" class="svws-ui-tr select-none text-uistatic" :style="getGridTemplateColumnsWithBackgroundColor(fachwahl)"
										...
										<template v-if="blockungstabelleHidden() === 'nichts'">
											<template v-if="setze_kursdifferenz(kurs).value && kurs_blockungsergebnis(kurs).value">
												<div role="cell" class="svws-ui-td svws-align-center cursor-pointer group relative" @click="toggle_active_fachwahl(kurs)">
													...
													<span class="icon-sm i-ri-filter-fill icon-uistatic absolute right-0 top-1" :class="(schuelerFilter().fach === kurs.fach_id) && (schuelerFilter().kursart?.id === kurs.kursart) ? 'text-ui-100' : 'invisible group-hover:visible opacity-25'" />
												</div>
										...
										<template v-if="blockungstabelleHidden() === 'schienen'">
											<div role="cell" class="svws-ui-td svws-align-center !p-[2px]">
												<div @click="toggleKursAusgewaehlt(kurs)"
													class="select-none w-full h-full rounded-xs flex justify-around items-center group bg-ui-100 text-ui-100 p-px"
											...
											<template v-for="(schiene, index) in getErgebnismanager().getMengeAllerSchienen()" :key="schiene.id">
												...
												<div role="cell" class="svws-ui-td svws-align-center !p-[2px]"
													:class="{
														...
														'bg-ui-0 opacity-50 text-ui-25 font-bold': highlightKursVerschieben(kurs).value,
													...
													<div v-if="istZugeordnetKursSchiene(kurs, schiene).value"
														...
														class="select-none w-full h-full rounded-xs flex justify-around items-center group p-px bg-ui-50 text-ui-75"
														:class="{
															'font-bold ring-ui-brand ring-3 text-ui-100': istKursAusgewaehlt(kurs).value,
															'cursor-grab': !isDragging,
														}">
														<span v-if="hatUpdateKompetenz && !istKursFixiertInSchiene(kurs, schiene).value" class="icon-sm group-hover:icon-ui-100 opacity-25 rounded-xs i-ri-draggable group-hover:opacity-100 px-1.5" />
														<svws-ui-tooltip v-if="getErgebnismanager().getOfKursAnzahlSchuelerExternePlusDummies(kurs.id) > 0">
															<span class="whitespace-nowrap">{{ getErgebnismanager().getOfKursAnzahlSchuelerPlusDummy(kurs.id) }}<span class="font-bold">*</span></span>
															<template #content>{{ getErgebnismanager().getOfKursAnzahlSchuelerInterne(kurs.id) }} und {{ getErgebnismanager().getOfKursAnzahlSchuelerExternePlusDummies(kurs.id) }} zusätzliche Kursteilnehmer</template>
														</svws-ui-tooltip>
									...
									<s-gost-kursplanung-kursansicht-kurs-details v-if="(kursdetailAnzeigen === kurs.id) && zeigeAufklappzeile" :bg-color="bgColor(fachwahl)" :anzahl-spalten="6 + getDatenmanager().schieneGetAnzahl()"
										:kurs :fachart="GostKursart.getFachartID(kurs.fach_id, kurs.kursart)" :get-datenmanager :api-status :get-ergebnismanager :map-lehrer :regeln-update :add-lehrer-regel :add-kurs :remove-kurse
										:add-kurs-lehrer :remove-kurs-lehrer :add-schiene-kurs :remove-schiene-kurs :split-kurs :combine-kurs />
								</template>
							</template>
						</template>
					</template>
...
<script setup lang="ts">
	...
	function setKursdetailAnzeigen(value: number | undefined) {
		kursdetailAnzeigen.value = kursdetailAnzeigen.value === value ? undefined : value;
	}
