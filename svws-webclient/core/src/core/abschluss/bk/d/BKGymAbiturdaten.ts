...
export class BKGymAbiturdaten extends JavaObject {
	 ...
	 */
	public idSchulgliederung: number = 0;
	 ...
	 */
	public readonly fachbelegungen: List<BKGymAbiturFachbelegung> = new ArrayList<BKGymAbiturFachbelegung>();
	...
	transpilerCanonicalName(): string {
		return 'de.svws_nrw.core.abschluss.bk.d.BKGymAbiturdaten';
	}
	...
	public static transpilerFromJSON(json: string): BKGymAbiturdaten {
		...
		const result = new BKGymAbiturdaten();
		...
		if (obj.fachbelegungen !== undefined) {
			for (const elem of obj.fachbelegungen) {
				result.fachbelegungen.add(BKGymAbiturFachbelegung.transpilerFromJSON(JSON.stringify(elem)));
			}
		}
	...
	public static transpilerToJSON(obj: BKGymAbiturdaten): string {
		let result = '{';
		...
		for (let i = 0; i < obj.fachbelegungen.size(); i++) {
			...
			result += BKGymAbiturFachbelegung.transpilerToJSON(elem);
	...
	public static transpilerToJSONPatch(obj: Partial<BKGymAbiturdaten>): string {
		let result = '{';
		if (obj.schuelerID !== undefined) {
			result += '"schuelerID" : ' + obj.schuelerID.toString() + ',';
		}
...
export function cast_de_svws_nrw_core_abschluss_bk_d_BKGymAbiturdaten(obj: unknown): BKGymAbiturdaten {
	return obj as BKGymAbiturdaten;
}
