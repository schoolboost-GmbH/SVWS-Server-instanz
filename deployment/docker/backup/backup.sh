...
while true; do
  ...
  # Backup all databases AS ROOT
  mysqldump -h mariadb -uroot -p"$MARIADB_ROOT_PASSWORD" \
  ...
  DBS=$(mysql -h mariadb -uroot -p"$MARIADB_ROOT_PASSWORD" -sNe "SHOW DATABASES;" | grep -Ev '^(mysql|information_schema|performance_schema|sys)$')
