...
# ----------------------------
# Generate TLS keystore if missing
# ----------------------------
...
if [ "$IMPORT_TEST_DATA" = "true" ]; then
    ...
    java -cp "../svws-server-app.jar:../*:../lib/*" de.svws_nrw.db.utils.app.ImportDB \
        -j -r -1 -td "MARIA_DB" \
        -f ${SQLITEFILE} \
        -tl ${MARIADB_HOST} \
        -ts ${MARIADB_DATABASE} \
...
# Dynamically find the JAR in case name changes
JAR_PATH=$(ls /opt/app/svws/*.jar | head -n1)
if [ -z "$JAR_PATH" ]; then
    echo "ERROR: SVWS JAR not found in /opt/app/svws/"
    exit 1
fi

echo "Using server JAR: $JAR_PATH"
java -jar "$JAR_PATH"
