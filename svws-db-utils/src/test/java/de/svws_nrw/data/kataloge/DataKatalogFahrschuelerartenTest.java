...
class DataKatalogFahrschuelerartenTest {
	...
	void setAttributesNotPatchableTest() {
		...
		when(this.conn.queryByKey(DTOFahrschuelerart.class, 1L)).thenReturn(dto);
	...
	void setAttributesRequiredOnCreationTest_bezeichnung() {
		try (var mapperMock = Mockito.mockStatic(JSONMapper.class)) {
			mapperMock.when(() -> JSONMapper.toMap(any(InputStream.class))).thenReturn(Map.of("sortierung", 3));
	...
	void getByIdTest_IdNull() {
		...
		assertThat(throwable)
				.isInstanceOf(ApiOperationException.class)
				.hasMessage("Die ID für die Fahrschülerart darf nicht null sein.")
				.hasFieldOrPropertyWithValue("status", Response.Status.BAD_REQUEST);
	}
	...
	void getByIdTest_IdNotFound() {
		...
		assertThat(throwable)
				.isInstanceOf(ApiOperationException.class)
				.hasMessage("Es wurde keine Fahrschülerart mit der ID 99 gefunden.")
	...
	void mapAttributeTest_bezeichnungDtoISNull() throws ApiOperationException {
		final var dto = new DTOFahrschuelerart(1L, "123");
		dto.Bezeichnung = null;
		final var newDto = new DTOFahrschuelerart(1L, "abc");
		when(conn.queryAll(DTOFahrschuelerart.class)).thenReturn(List.of(dto));
	...
	private static DTOFahrschuelerart getDto() {
		return new DTOFahrschuelerart(1L, "abc");
	}
}
