...
/** Diese Klasse testet die Klasse {@link DataSportbefreiungen}*/
@DisplayName("Diese Klasse testet die Klasse DataSportbefreiungen")
@ExtendWith(MockitoExtension.class)
class DataSportbefreiungenTest {

	@Mock
	private DBEntityManager conn;

	@InjectMocks
	private DataSportbefreiungen data;
	...
	void setAttributesNotPatchableTest() {
		...
		when(this.conn.queryByKey(DTOSportbefreiung.class, 1L)).thenReturn(dto);
	...
	void getByIdTest() throws ApiOperationException {
		...
		assertThat(this.data.getById(1L))
				.isInstanceOf(Sportbefreiung.class)
	...
	void getByIdTest_IdNull() {
		...
		assertThat(throwable)
				.isInstanceOf(ApiOperationException.class)
				.hasMessage("Die ID für die Sportbefreiung darf nicht null sein.")
				.hasFieldOrPropertyWithValue("status", Response.Status.BAD_REQUEST);
	}
	...
	void getByIdTest_IdNotFound() {
		...
		assertThat(throwable)
				.isInstanceOf(ApiOperationException.class)
				.hasMessage("Es wurde keine Sportbefreiung mit der ID 99 gefunden.")
	...
	void getAllTest() {
		...
		when(this.conn.queryAll(DTOSportbefreiung.class)).thenReturn(List.of(dto1, dto2));
	...
	private static DTOSportbefreiung getDto() {
		return new DTOSportbefreiung(1L, "abc");
	}
}
